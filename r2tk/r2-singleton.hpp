 ...
 * Updates:
 *  2013-04-04 (Rarosu):
 *      Updated GetDerivedAddress with typedefs for the pointer type.
 * 
 */
...
namespace r2 {
	...
	T* Singleton<T>::GetDerivedAddress(Singleton<T>* p_singleton_ptr) {
		...
		offset =	reinterpret_cast<PointerType>(reinterpret_cast<T*>( 1));
		offset -=	reinterpret_cast<PointerType>(reinterpret_cast<Singleton<T>*>( 1));

		// add the offset on the given singleton pointer, to yield the address of the derived class.
		return reinterpret_cast<T*>(reinterpret_cast<PointerType>( p_singleton_ptr + offset ));
	}
	...
	T* SingletonConst<T>::GetDerivedAddress(SingletonConst<T>* p_singleton_ptr) {
        ...
        typedef long PointerType;
		offset =	reinterpret_cast<PointerType>(reinterpret_cast<T*>( 1));
		offset -=	reinterpret_cast<PointerType>(reinterpret_cast<SingletonConst<T>*>( 1));
